...
static bool snowflakes_initialized = false;
...
void InitializeSnowflakes() {
    if (snowflakes_initialized) return;
    
    std::uniform_real_distribution<float> pos_dist_x(0.0f, 1920.0f);
    std::uniform_real_distribution<float> pos_dist_y(-1080.0f, 0.0f);
    std::uniform_real_distribution<float> size_dist(2.0f, 6.0f);
    std::uniform_real_distribution<float> speed_dist(20.0f, 80.0f);
    std::uniform_real_distribution<float> rotation_dist(0.0f, 360.0f);
    std::uniform_real_distribution<float> rotation_speed_dist(-180.0f, 180.0f);
    std::uniform_real_distribution<float> opacity_dist(0.3f, 0.9f);
    
    snowflakes.resize(150);
    for (auto& flake : snowflakes) {
        flake.position.x = pos_dist_x(rng);
        flake.position.y = pos_dist_y(rng);
        flake.size = size_dist(rng);
        flake.speed = speed_dist(rng);
        flake.rotation = rotation_dist(rng);
        flake.opacity = opacity_dist(rng);
        flake.rotation_speed = rotation_speed_dist(rng);
    }
    
    snowflakes_initialized = true;
}

void UpdateSnowflakes(float delta_time) {
    ImVec2 screen_size = ImGui::GetIO().DisplaySize;
    
    for (auto& flake : snowflakes) {
        flake.position.y += flake.speed * delta_time;
        flake.rotation += flake.rotation_speed * delta_time;
        
        if (flake.position.y > screen_size.y + 10.0f) {
            std::uniform_real_distribution<float> pos_dist_x(0.0f, screen_size.x);
            std::uniform_real_distribution<float> pos_dist_y(-50.0f, -10.0f);
...
void RenderSnowflakes() {
    if (!globals::misc::snowflakes) return;
    ...
    UpdateSnowflakes(delta_time);
    
    for (const auto& flake : snowflakes) {
        ImU32 color = IM_COL32(255, 255, 255, (int)(flake.opacity * 255));
        ImU32 glow_color = IM_COL32(200, 220, 255, (int)(flake.opacity * 100));
        
        draw_list->AddCircleFilled(flake.position, flake.size * 1.5f, glow_color);
        draw_list->AddCircleFilled(flake.position, flake.size, color);
        
        float half_size = flake.size * 0.7f;
        draw_list->AddLine(
            ImVec2(flake.position.x - half_size, flake.position.y),
            ImVec2(flake.position.x + half_size, flake.position.y),
            color, 1.0f
        );
        draw_list->AddLine(
            ImVec2(flake.position.x, flake.position.y - half_size),
            ImVec2(flake.position.x, flake.position.y + half_size),
            color, 1.0f
        );
    }
...
void RenderCustomCursor() {
    if (!globals::misc::customcursor) return;
    ...
    float cursor_size = 20.0f;
    ImU32 cursor_color = IM_COL32(255, 100, 100, 255);
    ImU32 cursor_outline = IM_COL32(255, 255, 255, 200);
    
    draw_list->AddCircleFilled(mouse_pos, cursor_size * 0.6f, cursor_outline);
    draw_list->AddCircleFilled(mouse_pos, cursor_size * 0.4f, cursor_color);
    
    float cross_size = cursor_size * 0.8f;
    draw_list->AddLine(
        ImVec2(mouse_pos.x - cross_size, mouse_pos.y),
        ImVec2(mouse_pos.x + cross_size, mouse_pos.y),
        cursor_outline, 2.0f
    );
    draw_list->AddLine(
        ImVec2(mouse_pos.x, mouse_pos.y - cross_size),
        ImVec2(mouse_pos.x, mouse_pos.y + cross_size),
        cursor_outline, 2.0f
    );
    
    draw_list->AddCircleFilled(mouse_pos, 3.0f, cursor_outline);
    
    io.MouseDrawCursor = false;
}

void LoadCustomCursor() {
    std::cout << "[CURSOR] Custom cursor loaded (placeholder)" << std::endl;
}
...
void render_watermark() {
    ...
    std::string watermark_text = "Vyre Premium";
    ...
    while (done == false)
    {
        ...
        if (FindWindowA(0, "Roblox") && (GetForegroundWindow() == FindWindowA(0, "Roblox") || GetForegroundWindow() == hwnd))
        {
            ...
            if (overlay::visible)
            {
                ...
                ImAdd::Text(style.Colors[ImGuiCol_Text], "Vyre Premium");
                ...
                const char* tabNames[] = { "Combat", "Silent Aim", "Triggerbot", "Visuals", "Rage", "Settings", "Credits" };
                ...
                for (int i = 0; i < 7; i++) {
                    ...
                    if (i < 6) totalTabWidth += tabSpacing;
                }
                ...
                for (int i = 0; i < 7; i++) {
                    ...
                    if (isActive) {
                        ...
                        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
                        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.1f, 0.1f, 0.1f, 0.4f));
                    ...
                    if (i < 6) ImGui::SameLine(0, tabSpacing);
                }

                float tabBottomY = 4.0f + 20.0f;
                ...
                ImGui::BeginGroup(); {
                        ...
                        ImGui::BeginGroup();
                        {
                            ImGui::BeginGroup();
                            {
                                ImAdd::BeginChild("ESP", ImVec2(window_width - 20 - 8, 550));
                                {
                                    ...
                                    ImAdd::SliderFloat("Render Distance", &globals::visuals::renderdistance, 100, 5000);
                        ...
                        ImGui::BeginGroup();
                        {
                            ...
                            ImGui::BeginGroup();
                            {
                                ...
                                ImAdd::BeginChild("PlayerMods", ImVec2((window_width - 20) / 2 - 8, 200));
                                {
                                    ...
                                    ImAdd::CheckBox("AutoSlide", &globals::misc::autoslide);
                    ...
                    if (iTabID == 5) {
                        ImGui::BeginGroup();
                        {
                            ImGui::BeginGroup();
                            {
                                ImAdd::BeginChild("Settings", ImVec2((window_width - 20) / 2 - 8, 200));
                                {
                                    draw_shadowed_text("Settings");
                                    ImAdd::CheckBox("Watermark", &globals::misc::watermark);
                                    ImAdd::CheckBox("Playerlist", &globals::misc::playerlist);
                                    ImAdd::CheckBox("Streamproof", &globals::misc::streamproof);
                                    ImAdd::CheckBox("Knock Check", &globals::combat::knockcheck);
                                    ImAdd::CheckBox("Snowflakes", &globals::misc::snowflakes);
                                    ImAdd::CheckBox("Custom Cursor", &globals::misc::customcursor);
                                }
                                ImAdd::EndChild();
                            }
                            ImGui::EndGroup();
                            ...
                            ImGui::BeginGroup();
                            {
                                ImAdd::BeginChild("Config", ImVec2((window_width - 20) / 2 - 8, 200));
                                {
                                    draw_shadowed_text("Config");
                                    
                                    static char config_name[128] = "";
                                    ImGui::InputText("Config Name", config_name, sizeof(config_name));
                                    
                                    if (ImAdd::Button("Save Config", ImVec2(ImGui::GetContentRegionAvail().x, 25))) {
                                        if (strlen(config_name) > 0) {
                                            g_config_system.save_config(config_name);
                                            Notifications::Success("Config saved!");
                                        }
                                    }
                                    ...
                                    g_config_system.refresh_config_list();
                                    std::vector<const char*> config_names;
                                    for (const auto& config : g_config_system.config_files) {
                                        config_names.push_back(config.c_str());
                                    }
                                    
                                    if (!config_names.empty()) {
                                        ImGui::Combo("Load Config", &selected_config, config_names.data(), (int)config_names.size());
                                        
                                        if (ImGui::Button("Load Selected", ImVec2(ImGui::GetContentRegionAvail().x, 25))) {
                                            if (selected_config >= 0 && selected_config < g_config_system.config_files.size()) {
                                                g_config_system.load_config(g_config_system.config_files[selected_config]);
                                                Notifications::Success("Config loaded!");
                                            }
                                        }
                                    } else {
                    ...
                    if (iTabID == 6) {
                        ImGui::BeginGroup();
                        {
                            ImAdd::BeginChild("Credits", ImVec2(window_width - 20, 550));
                            {
                                draw_shadowed_text("Credits");
                                ImGui::Text("Credits to Treaki for developing Vyre Premium:");
                                ImGui::Separator();
                                ImGui::Text("YouTube Channel:");
                                ImGui::SameLine();
                                ImGui::TextLinkOpenURL("https://www.youtube.com/@Treakiii", "https://www.youtube.com/@Treakiii");
                                ImGui::Text("Discord Server:");
                                ImGui::SameLine();
                                ImGui::TextLinkOpenURL("https://discord.com/invite/ACa6bCd6xx", "https://discord.com/invite/ACa6bCd6xx");
                                ImGui::Text("GitHub Profile:");
                                ImGui::SameLine();
                                ImGui::TextLinkOpenURL("https://github.com/treakiii", "https://github.com/treakiii");
                            }
                            ImAdd::EndChild();
                        }
                        ImGui::EndGroup();
        ...
        // Update and render snowflakes if enabled
        if (globals::misc::snowflakes) {
            if (!snowflakes_initialized) {
                InitializeSnowflakes();
                snowflakes_initialized = true;
            }
            UpdateSnowflakes(ImGui::GetIO().DeltaTime);
            RenderSnowflakes();
        }

        // Render custom cursor if enabled
        if (globals::misc::customcursor) {
            RenderCustomCursor();
        }
